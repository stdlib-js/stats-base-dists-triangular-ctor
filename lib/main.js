...
function Triangular() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( a ) || isnan( a ) ) {
			throw new TypeError( format( '18d8a', a ) );
		}
		if ( !isNumber( b ) || isnan( b ) ) {
			throw new TypeError( format( '18d8b', b ) );
		}
		if ( !isNumber( c ) || isnan( c ) ) {
			throw new TypeError( format( '18d9B', c ) );
		}
		if ( !( a <= c && c <= b ) ) {
			throw new RangeError( format( '18d9C', 'a <= c <= b', a, b, c ) );
		}
	} else {
		a = 0.0;
		b = 1.0;
	...
	defineProperty( this, 'a', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '18d8d', value ) );
			}
			if ( value > b || value > c ) {
				throw new RangeError( format( '18d9D', value ) );
			}
			a = value;
		}
	});
	defineProperty( this, 'b', {
		...
		'set': function set( value ) {
			...
			if ( a > value || c > value ) {
				throw new RangeError( format( '18d9E', value ) );
			}
			b = value;
		}
	});
	defineProperty( this, 'c', {
		...
		'set': function set( value ) {
			...
			if ( a > value || b < value ) {
				throw new RangeError( format( '18d9F', value ) );
			}
			c = value;
		}
	});
